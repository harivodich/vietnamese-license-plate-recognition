...
def test_ocr_checkpoint_selection_prioritizes_cer_over_sparse_exact_match() -> None:
    """Khi exact còn hiếm, CER thấp hơn chọn checkpoint ổn định hơn."""
...
def test_augmentation_for_epoch_respects_warmup() -> None:
    """Augmentation chỉ bật sau warmup để model học ký tự sạch trước."""
    from vlpr.training.ocr import _augmentation_for_epoch
    from vlpr.training.ocr_config import (
        OcrAugmentationSettings,
        OcrCrnnModelSettings,
        OcrOptimizationSettings,
        OcrOutputSettings,
    ...
    config = OcrTrainingExperimentConfig(
        ...
        train=OcrOptimizationSettings(
            ...
            augmentation_warmup_epochs=12,
            weight_decay=0.0001,
            gradient_clip_norm=5.0,
            patience=15,
            save_period=5,
        ),
        output=OcrOutputSettings(project=Path("artifacts"), name="ocr"),
    )

    assert _augmentation_for_epoch(config, 12) is None
    assert _augmentation_for_epoch(config, 13) == config.augmentation
